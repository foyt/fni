...
if [[ $start_sc_tunnel = true ]]; then
  curl -sS https://saucelabs.com/downloads/sc-4.3.6-linux.tar.gz|tar -xzC /tmp/
  /tmp/sc-4.3.6-linux/bin/sc -r 10 -u $SAUCE_USERNAME -k $SAUCE_ACCESS_KEY -i $TRAVIS_JOB_NUMBER --vm-version dev-varnish -B *.facebook.com --daemonize --readyfile /tmp/sc-ready
  t=0;
