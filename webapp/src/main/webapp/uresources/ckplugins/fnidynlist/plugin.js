  ...
  onLoad : function() { 
    ...
    CKEDITOR.tools.extend(dynListElement.prototype, {
      ...
      getButtonElement: function (row, id) {
        return document.getElementById(this.domId + '.' + id + '.' + row);
      },
      getRowElement: function (row) {
        return document.getElementById(this.domId + '.row.' + row);
      },
      getContentElement: function () {
        return document.getElementById(this.domId + '_content');
      },
      ...
      getCellValue: function (row, column) {
        switch (this.columns[column].type) {
          case 'checkbox':
            return document.getElementById(this.domId + '.cell.' + column + '.' + row).checked;
          break;
          case 'hidden':
            return document.getElementById(this.domId + '.cell.' + column + '.' + row).value;            
          break;
          case 'icon':
            return document.getElementById(this.domId + '.cell.' + column + '.' + row).src;            
          break;
          default:
      ...
      setCellValue: function (row, column, value) {
        switch (this.columns[column].type) {
          case 'checkbox':
            document.getElementById(this.domId + '.cell.' + column + '.' + row).checked = value;
          break;
          case 'hidden':
            document.getElementById(this.domId + '.cell.' + column + '.' + row).value = value;            
          break;
          case 'icon':
            document.getElementById(this.domId + '.cell.' + column + '.' + row).src = value;
          break;
          default:
            document.getElementById(this.domId + '.cell.' + column + '.' + row).innerHTML = value;
          break;
        }
      },
      ...
      startLoading: function () {
        this.getDOMElement().origClassName = this.getDOMElement().className;
        this.getDOMElement().className += ' fniDynListLoading';
        if (this._emptyNode) {
      ...
      stopLoading: function () {
        if (this.getDOMElement().origClassName) {
          this.getDOMElement().className = this.getDOMElement().origClassName;
          delete this.getDOMElement().origClassName;
        }
      ...
      _changeRowId: function (oldId, newId) {
        ...
        for (var i = 0; i < this.columns.length; i++) {
          var valueElement = document.getElementById(this.domId + '.cell.' + i + '.' + oldId);
          if (valueElement)
        ...
        if (this.buttons) {
          for (var b = 0; b < this.buttons.length; b++) {
            var button = this.buttons[b];
            var buttonElement = document.getElementById(this.domId + '.' + button.id + '.' + oldId);
