...
<c:choose>
  <c:when test="${empty(param.id)}">
    ...
    <c:set var="fieldId" value="form-memo-field-${uuid.uniqueId}"/>  
  </c:when>
  <c:otherwise>
...
<div class="formField formMemoField">
  <div class="formFieldLabelContainer">
    <c:choose>
      <c:when test="${!empty(param.labelLocale)}">
        <label for="${fieldId}">
          <fmt:message key="${param.labelLocale}"></fmt:message>
        </label>
      </c:when>
      <c:otherwise>
  ...
  <div class="formFieldEditorContainer">
    <textarea id="${fieldId}" name="${param.name}" class="${param.classes}">${fn:escapeXml(param.value)}</textarea>
  </div>
</div>