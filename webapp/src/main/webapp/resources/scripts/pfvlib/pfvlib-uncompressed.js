 ...
 * Min field validator.
 * 
 * Validates that number equals or is more than specified value. Value is defined by adding min-VALUE class to the element.
 *  
 * Copyright (C) 2011 Antti Lepp� / Foyt
 * http://www.foyt.fi
 * 
 * License: 
 * 
 ...
 * http://www.gnu.org/licenses/lgpl.html
 */
fi.internetix.validation.validator.MinValidValidator = Class.create(fi.internetix.validation.FieldValidator,
  /** @lends fi.internetix.validation.validator.MinValidValidator# **/   
  {
  /**
  * Class constructor
  * @class Constructs this class
  ...
  */
  ...
  validate: function ($super, field) {
    ...
    var fieldClassNames = $w(field.className);
    for (var i = 0, l = fieldClassNames.length; i < l; i++) {
      if (fieldClassNames[i].startsWith('min-')) {
        minValue = parseFloat(fieldClassNames[i].substring(4));
        break;
      }
    }
    
    if (minValue !== null) {
      var value = parseFloat(this._getFieldValue(field));
      if (Object.isNumber(value) && !isNaN(value)) {
        return value >= minValue ? fi.internetix.validation.FieldValidator.STATUS_VALID : fi.internetix.validation.FieldValidator.STATUS_INVALID;
      }
    }
...
fi.internetix.validation.FieldValidatorVault.registerValidator(new fi.internetix.validation.validator.MinValidValidator());/*
 * Max field validator.
 * 
 * Validates that number equals or is less than specified value. Value is defined by adding max-VALUE class to the element.
 *  
 * Copyright (C) 2011 Antti Lepp� / Foyt
 * http://www.foyt.fi
 * 
 ...
 * http://www.gnu.org/licenses/lgpl.html
 */
fi.internetix.validation.validator.MaxValidValidator = Class.create(fi.internetix.validation.FieldValidator,
  /** @lends fi.internetix.validation.validator.MaxValidValidator# **/   
  {
  /**
  * Class constructor
  * @class Constructs this class
  ...
  */
  ...
  validate: function ($super, field) {
    ...
    var fieldClassNames = $w(field.className);
    for (var i = 0, l = fieldClassNames.length; i < l; i++) {
      if (fieldClassNames[i].startsWith('max-')) {
        maxValue = parseFloat(fieldClassNames[i].substring(4));
        break;
      }
    }
    ...
    if (maxValue !== null) {
      var value = parseFloat(this._getFieldValue(field));
      if (Object.isNumber(value) && !isNaN(value)) {
        return value <= maxValue ? fi.internetix.validation.FieldValidator.STATUS_VALID : fi.internetix.validation.FieldValidator.STATUS_INVALID;
      }
    }
...
fi.internetix.validation.FieldValidatorVault.registerValidator(new fi.internetix.validation.validator.MaxValidValidator());