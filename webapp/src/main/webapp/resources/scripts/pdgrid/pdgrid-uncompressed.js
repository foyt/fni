...
function getDataGrids() {
  ...
  insertRow: function (values, beforeRow, editable) {
    var oldRowCount = this.getRowCount();
    var refRow = this._rowElements.get(beforeRow);
    
    for (var row = oldRowCount - 1; row >= beforeRow ; row--) {
      this._changeRowNumber(row, row + 1);
    }

    var newRow = this._createRow(values, beforeRow, editable);
    this._rowElements.set(beforeRow, newRow);
    
    if (!refRow) {
      this._content.appendChild(newRow);
    } else {
      refRow.insert({
        before: newRow
      });
    ...
    this._setRowCount(oldRowCount + 1);
  },
  switchRows: function (row1, row2) {
    var oldRowElement1 = this.getRowElement(row1);
    var oldRowElement2 = this.getRowElement(row2);
    var newBefore1 = this.getRowElement(row2 + 1);
    var newBefore2 = this.getRowElement(row1 + 1);
    
    var tempRow = this.getRowCount() + 1;
    this._changeRowNumber(row1, tempRow);
    this._changeRowNumber(row2, row1);
    this._changeRowNumber(tempRow, row2);
    
    if (newBefore1) {
      this._content.insertBefore(oldRowElement1, newBefore1);
    } else {
      this._content.appendChild(oldRowElement1);
    }
    
    if (newBefore2) {
      this._content.insertBefore(oldRowElement2, newBefore2);
    } else {
      this._content.appendChild(oldRowElement2);
    }
  },
  _createRow: function (values, rowNumber, editable) {
  ...
  _addRows: function (rowDatas, editable) {
    ...
    for (var rowIndex = 0, rowCount = rowDatas.length; rowIndex < rowCount; rowIndex++) {
      ...
      rowElements.push(this._createRow(values, rowNumber, editable));
    ...
    for (var i = 0, l = rowElements.length; i < l; i++) {
      ...
      result = rowElements[i]._rowNumber;
    ...
    if (this.fire("dataGridIdChange", { oldId: oldId, newId: newId})) {
      ...
      for (var i = 0; i < this.options.columns.length; i++) {
        for (var row = 0; row < this.getRowCount(); row++) {
          var column = this.options.columns[i];
          this._changeCellId(newId ? newId + '.' + row + '.' + (column.paramName ? column.paramName : i) : '');
        }
      }
    };
  },
  _changeRowNumber: function (row, newRow) {
    var rowElement = this._rowElements.get(row);
    this._rowElements.unset(row);
    rowElement._rowNumber = newRow;
    this._rowElements.set(newRow, rowElement);

    for (var columnIndex = 0; columnIndex < this.options.columns.length; columnIndex++) {
      // Change cell's id 
      var column = this.options.columns[columnIndex];
      var cellId = this.options.id ? this.options.id + '.' + newRow + '.' + (column.paramName ? column.paramName : columnIndex) : '';
      this._changeCellId(row, columnIndex, cellId);
      // Switch it's handler into appropriate place
      var handlerInstance = this.getCellEditor(row, columnIndex);
      this._unsetCellContentHandler(row, columnIndex);
      this._setCellContentHandler(newRow, columnIndex, handlerInstance);
    }    
  },
  _changeCellId: function (row, column, newId) {
    var cellEditor = this.getCellEditor(row, column);
    DataGridControllers.getController(cellEditor._dataType).changeParamName(cellEditor, newId);
  },
